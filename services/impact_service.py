...
def analyze_impact(
    ...
    candidate_rows = resolve_candidates(duckdb_store, target=target, file_path=file_path, kind=kind, symbol_uid_value=symbol_uid, limit=25)
    ...
    process_participation = _process_participation(duckdb_store, resolved_target)
    return {
        "target": {
            "name": target_symbol.get("name", target),
            "qualified_name": resolved_target,
            "kind": target_symbol.get("kind", ""),
            "uid": target_symbol.get("uid", ""),
